...
void *ft_mlx_new_window(mlx_ptr_t *mlx_ptr, int pos_x, int pos_y, int size_x, int size_y, char *title)
{
  mlx_win_list_t	*newwin;
  NSString		*str;
  ...
  NSRect windowRect = NSMakeRect(pos_x, [[NSScreen mainScreen] frame].size.height - size_y - pos_y - 45, size_x, size_y);
  str = [NSString stringWithCString:title encoding:NSASCIIStringEncoding];
  newwin->winid = [[MlxWin alloc] initWithRect:windowRect andTitle:str pfaAttrs:pfa_attrs];
  if (newwin->winid)
    if (![(id)(newwin->winid) pixel_management])
